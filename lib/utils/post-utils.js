...
const { dbGetUsers } = require("./user-utils");
  ...
  try {
    ...
    if (sqlResult) {
      ...
      for (const record of records) {
        //collect user data.
        const [sender] = await dbGetUsers({ UserId: record.SenderId });
        const [receiver] = await dbGetUsers({ UserId: record.ReceiverId });
 ...
 * @returns
 */
const dbAddPost = async ({ SenderId, ReceiverId, PostHtml }) => {
  try {
    let senderId = SenderId || "";
    let receiverId = ReceiverId || "";
 ...
 * @returns
 */
const dbDeletePost = async ({ PostId }) => {
  try {
    let postId = PostId || "";
...
module.exports = { dbGetPosts, dbAddPost, dbDeletePost };
