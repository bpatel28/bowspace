const { dbGetUsers, dbAddNewUser, dbUpdateUserInfo } = require("../utils/user-utils");
...
const searchUsers = async (req, res) => {
  try {
    const users = await dbGetUsers(req.query);
...
const registerUser = async (req, res) => {
  try {
    const user = await dbAddNewUser(req.body);
...
const updateUser = async (req, res) => {
  try {
    const user = await dbUpdateUserInfo(req.body);
